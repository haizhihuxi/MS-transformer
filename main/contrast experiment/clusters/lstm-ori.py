...
# 假设数据集中每个轨迹是一个形状为 (time_steps, features) 的 numpy 数组
# 且每个数据集包含多个这样的轨迹
...
if reset:
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath='best_ship_trajectory_lstm_model.keras',
        save_best_only=True,
        monitor='val_loss',
        mode='min',
        verbose=1
...
# 加载最好的模型
best_model = load_model('best_ship_trajectory_lstm_model.keras')
