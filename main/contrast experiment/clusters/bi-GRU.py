...
from tensorflow.keras.layers import GRU, Dense, Input, Bidirectional, Concatenate, TimeDistributed, Reshape
...
if reset:
    ...
    # 编码器：BiGRU层
    encoder_bigru = Bidirectional(GRU(64, return_sequences=True, return_state=True))
    encoder_outputs, forward_h, backward_h = encoder_bigru(encoder_inputs)
    state_h = Concatenate()([forward_h, backward_h])
    # 重复向量，将时间步长扩展到 82
    encoder_expanded = TimeDistributed(Dense(24))(encoder_outputs)  # (None, 18, 24)

    # 变换形状以匹配解码器输入
    encoder_reshaped = Reshape((24, 18))(encoder_expanded)  # (None, 24, 128)

    # TimeDistributed层将特征维度转换为 2
    encoder_dense = TimeDistributed(Dense(2))(encoder_reshaped)  # (None, 24, 2)

    # 解码器：单向GRU层
    decoder_gru = GRU(128, return_sequences=True, return_state=True)
    decoder_outputs, _ = decoder_gru(encoder_dense, initial_state=state_h)
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath='best_ship_trajectory_bigru_model.keras',
        save_best_only=True,
        monitor='val_loss',
        mode='min',
        verbose=1
...
# 加载最好的模型
best_model = load_model('best_ship_trajectory_bigru_model.keras')
