...
from tensorflow.keras.layers import LSTM, Dense, Input, Bidirectional, Concatenate, Attention, TimeDistributed, Reshape
...
if reset:
    # 输入层
    encoder_inputs = Input(shape=(18, 2))

    # 编码器：BiLSTM层
    encoder_bilstm = Bidirectional(LSTM(64, return_sequences=True, return_state=True))
    ...
    # 调整注意力输出的时间步长
    # 重复向量，将时间步长扩展到 82
    attention_expanded = TimeDistributed(Dense(24))(attention)  # (None, 18, 24)

    # 变换形状以匹配解码器输入
    attention_reshaped = Reshape((24, 18))(attention_expanded)  # (None, 24, 128)

    # TimeDistributed层将特征维度转换为 2
    attention_dense = TimeDistributed(Dense(2))(attention_reshaped)  # (None, 24, 2)

    # 解码器：单向LSTM层
    decoder_lstm = LSTM(128, return_sequences=True, return_state=True)
    decoder_outputs, _, _ = decoder_lstm(attention_dense, initial_state=[state_h, state_c])

    # 最后一个全连接层
    final_output = TimeDistributed(Dense(2))(decoder_outputs)
    ...
    y_train = y_train.reshape((y_train.shape[0], 24, X_train.shape[2]))
    y_valid = y_valid.reshape((y_valid.shape[0], 24, X_train.shape[2]))

    # 定义ModelCheckpoint回调以保存验证集上表现最好的模型
    checkpoint_callback = ModelCheckpoint(
        filepath='best_ship_trajectory_bilstm_attention_model.keras',
        save_best_only=True,
        monitor='val_loss',
        mode='min',
        verbose=1
...
# 加载最好的模型
best_model = load_model('best_ship_trajectory_bilstm_attention_model.keras')
...
# 创建结果文件夹
results_folder = 'lstm-results/bilstm_attention'
