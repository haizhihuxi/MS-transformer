...
model = LSTMModel(input_size=single_traj.shape[1]).to(device)
...
# 将预测结果从(1, 48)的形状转换为(24, 2)
y_single_pred = y_single_pred.reshape(-1, 2)
...
pre_tru_lat = denormalize_coords(single_traj[18:, 1], lat_min, lat_max)
pre_tru_lon = denormalize_coords(single_traj[18:, 0], lon_min, lon_max)
pred_lat = denormalize_coords(y_single_pred[:, 1], lat_min, lat_max)
pred_lon = denormalize_coords(y_single_pred[:, 0], lon_min, lon_max)

savepath = '../picture-results/测试组/LSTM predict .png'
plt.plot(inputs_lat, inputs_lon, 'g-', label='Input')  # 绘制输入轨迹
plt.plot(pre_tru_lat, pre_tru_lon, 'b-', label='True')  # 绘制输入轨迹
